...
def parse(path='chinese.txt'):
    ...
    with open('chinese_hieroglyphs.txt', 'w', encoding="utf-8") as hieroglyphs,\
        ...
        for i in range(len(lines)):
            ...
            pinyin_list = []
            if re.search(r'([(（]|)[a-zA-ZāēīōūǖáéíóúǘǎěǐǒǔǚàèìòùǜüĀĒĪŌŪǕÁÉÍÓÚǗǍĚǏǑǓǙÀÈÌÒÙǛÜǹǸ]([)）]|)', current_line):
                for j in re.finditer(r'([(（]|)[a-zA-ZāēīōūǖáéíóúǘǎěǐǒǔǚàèìòùǜüĀĒĪŌŪǕÁÉÍÓÚǗǍĚǏǑǓǙÀÈÌÒÙǛÜǹǸ]([)）]|)', current_line):
